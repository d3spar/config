...
local update_volume = function()
	...
	awful.spawn.easy_async_with_shell("pactl get-sink-mute @DEFAULT_SINK@", function(is_muted)
		...
		if is_muted then
			message = message .. "<span foreground='#ff0000'><big><b>M</b></big></span>"
			volume_widget:set_markup_silently(message)
		else
			awful.spawn.easy_async_with_shell(
				[[pactl get-sink-volume @DEFAULT_SINK@ | awk '{print $5}']],
				function(actual_volume)
...
awesome.connect_signal("widgets::volume::update", update_volume)
--awesome.emit_signal("widgets::volume::update") -- initial update
gears.timer({
	timeout = 60,
	call_now = true,
	autostart = true,
...
--beautiful.wallpaper = "/home/esh/eshBG.jpg"
...
mymainmenu = awful.menu({
	items = {
		...
		{ "files", "thunar" }, --"nautilus --new-window" },
	},
})
...
month_cal:attach(mytextclock, "tr")
...
clientkeys = gears.table.join(
	...
	awful.key({}, "XF86AudioRaiseVolume", function()
		awful.spawn.easy_async_with_shell("pactl set-sink-volume @DEFAULT_SINK@ +2%", function()
			awesome.emit_signal("widgets::volume::update")
		end)
	end),
...
awful.rules.rules = {
	...
	-- Floating clients.
	{
		rule_any = {
			...
			name = {
				"Event Tester", -- xev.
				"PrimeVideo - Chat - Twitch — Mozilla Firefox",
				"*.Chat - Twitch.*",
			},
			role = {
	...
	--  properties = { tag = tags[1][5] } },
	{
		...
		properties = { screen = 1, tag = "4", placement = awful.placement.bottom_right },
	},
}
-- }}}
...
--awful.spawn.with_shell("gwe")
--awful.spawn.once("kdeconnect-app")
